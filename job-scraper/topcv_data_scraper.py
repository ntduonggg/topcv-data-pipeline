...
def parse_search_page(session: requests.Session, url: str) -> List[Dict]:
    ...
    for job in soup.select("div.job-item-search-result"):
        ...
        if not a_title:
            continue
        title = text(a_title)
        job_url = urljoin(URL_BASE, a_title.get("href"))

        comp_a = job.select_one("a.company[href]")
        company = text(job.select_one("a.company .company-name"))
        company_url = urljoin(URL_BASE, comp_a.get("href")) if comp_a else None
...
def extract_company_link_from_job(soup: BeautifulSoup) -> Optional[str]:
    ...
    return urljoin(URL_BASE, cand["href"]) if cand and cand.has_attr("href") else None
...
if __name__ == "__main__":
    qtpl = "https://www.topcv.vn/tim-viec-lam-data-engineer-tai-ho-chi-minh-kl2?type_keyword=1&page={page}&locations=l2&saturday_status=0&sba=1"
    ...
    print(df.head())
    df.to_csv("../data-files/topcv_data_jobs.csv", index=False, encoding="utf-8-sig")
    print("Saved CSV: topcv_data_jobs.csv")

    df.to_excel("../data-files/topcv_data_jobs.xlsx")
    print("Saved Excel: topcv_data_jobs.xlsx")